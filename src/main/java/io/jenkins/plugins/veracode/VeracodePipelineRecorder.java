...
public class VeracodePipelineRecorder extends Recorder implements SimpleBuildStep {
    ...
    */
	...
	public VeracodePipelineRecorder(String applicationName, String criticality, String sandboxName, String scanName, boolean waitForScan, int timeout,
			boolean createProfile, String teams, boolean createSandbox, boolean timeoutFailsJob, boolean canFailJob, boolean debug,
			String uploadIncludesPattern, String uploadExcludesPattern, String scanIncludesPattern, String scanExcludesPattern, String fileNamePattern,
			String replacementPattern, boolean copyRemoteFiles, boolean useProxy, String pHost, String pPort, String pUser, String pPassword,
			String vid, String vkey) {
	...
	public void perform(Run<?, ?> run, FilePath workspace, Launcher launcher, TaskListener listener)
		...
		try {
			...
			//
			// Placeholders
			// We may want to implement these in the future, but I don't think it is necessary for pipeline.
			// The only thing we're really missing is autoapplicationdescription. We may want to allow the user to update the veracode description of the application.
			// Currently createAutoApplicationDescription = true will not resolve the host name for Jenkins properly on jenkins 2.x. Not sure why.
			///
			...
			UploadAndScanArgs uploadAndScanArguments = UploadAndScanArgs.newUploadAndScanArgs(false, autoApplicationName, createAutoApplicationDescription, autoScanName, createSandbox,
					createProfile, teams, useProxy, vid, vkey, run.getDisplayName(),
					run.getParent().getFullDisplayName(), applicationName, sandboxName, scanName, criticality,
					scanIncludesPattern, scanExcludesPattern, fileNamePattern, replacementPattern, pHost,
					pPort, pUser, pPassword, workspace, run.getEnvironment(listener), str_timeout, uploadAndScanFilePaths);
	...
	@Symbol("veracode")
	@hudson.Extension public static final class PipelineDescriptorImpl extends BuildStepDescriptor<Publisher> {
    ...
    private boolean runScanFromRemote(Run<?, ?> run, FilePath workspace, TaskListener listener, PrintStream ps ) throws IOException, InterruptedException {
        ...
        try {
            ...
            UploadAndScanArgs uploadAndScanArguments = UploadAndScanArgs.newUploadAndScanArgs(true, autoApplicationName, createAutoApplicationDescription, autoScanName, createSandbox,
                    createProfile, teams, useProxy, vid, vkey, run.getDisplayName(),
                    run.getParent().getFullDisplayName(), applicationName, sandboxName, scanName, criticality,
                    scanIncludesPattern, scanExcludesPattern, fileNamePattern, replacementPattern, pHost,
                    pPort, pUser, pPassword, workspace, run.getEnvironment(listener), str_timeout, uploadAndScanFilePaths);
            ...
            for (int i =0; i<iSize; i++) {
                if (iPosPassword != -1) {
                    if (iPosPassword == i) masks[i]=true;
                } else if (iPosKey != -1) {
    ...
    private void getScanResults(Run<?, ?> run, TaskListener listener, boolean autoApplicationName) throws Exception {
        ...
        String resolvedSandboxName = !StringUtil.isNullOrEmpty(sandboxName) ? envVars.expand(sandboxName) : sandboxName;
        try {
            String buildInfoXML = WrapperUtil.getBuildInfo(appName, resolvedSandboxName, vid, vkey, proxy);
