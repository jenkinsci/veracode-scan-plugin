...
public final class RemoteScanUtil {
	...
	//getting the jar version so that we can decide to copy the latest
		...
		public static Integer[] getMaskPosition (List <String> remoteCmd )
		{
			...
			Integer[] maskPos;
			maskPos = new Integer[] {-1,-1,-1};

			int i =0;
			for (String _cmd: remoteCmd)
			{
				if(_cmd.equals("-vkey")) maskPos[1] = i+1;
				else if(_cmd.equals("-ppassword")) maskPos[2] = i+1;
				i++;
			}
	    ...
	    /**
		 * Edit parameter values to avoid splitting if they contain spaces in pipeline with remote scan
