...
public class DynamicAnalysisResultsPipelineRecorder extends Recorder implements SimpleBuildStep {
	...
	public DynamicAnalysisResultsPipelineRecorder(int waitForResultsDuration, boolean failBuildForPolicyViolation,
			boolean debug, boolean useProxy, String pHost, String pPort, String pUser, String pPassword, String vid,
			String vkey) {
    ...
    public void perform(Run<?, ?> run, FilePath workspace, Launcher launcher, TaskListener listener)
		...
		boolean buildSuccess = daAdapterService.reviewDynamicAnalysis(run, workspace, listener, waitForResultsDuration,
				failBuildForPolicyViolation, vid, vkey, debug, proxyBlock);
    ...
    @Extension
    public static final class PipelineDynamicAnalysisResultsDescriptorImpl extends BuildStepDescriptor<Publisher> {
		...
		public Publisher newInstance(StaplerRequest req, JSONObject formData) throws FormException {
			...
			formData.put("waitForResultsDuration",
					StringUtil.isNullOrEmpty(waitForResultsDuration)
							? TimeUnit.MINUTES.toHours(Constant.DEFAULT_VALUE_DA_WAIT_FOR_RESULTS_MINUTES)
        ...
        // Validate proxy host
        public FormValidation doCheckPHost(@QueryParameter("pHost") String pHost, @QueryParameter("pPort") String pPort)
               throws IOException, ServletException {
            return FormValidationUtil.checkFields(pHost, pPort, "Host", "Port", null);
       }

        // Validate proxy port
        public FormValidation doCheckPPort(@QueryParameter("pHost") String pHost, @QueryParameter("pPort") String pPort)
                throws IOException, ServletException {
            return FormValidationUtil.checkFields(pPort, pHost, "Port", "Host", null);
        }

        // Validate proxy username
        public FormValidation doCheckPUser(@QueryParameter("pUser") String pUser, @QueryParameter("pPassword") String pPassword)
                throws IOException, ServletException {
            return FormValidationUtil.checkMutuallyInclusiveFields(pUser, pPassword, "User", "Password", null);
        }

        // Validate proxy password
        public FormValidation doCheckPPassword(@QueryParameter("pUser") String pUser, @QueryParameter("pPassword") String pPassword)
               throws IOException, ServletException {
