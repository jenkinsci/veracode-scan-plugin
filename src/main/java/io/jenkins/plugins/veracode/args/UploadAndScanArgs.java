...
public final class UploadAndScanArgs extends AbstractArgs {
	...
	/**
	 * Adds the specified arguments and switches for those arguments to the
	...
	private void addStdArguments(boolean bRemoteScan, String appname, String description, boolean createprofile, String teams, String criticality, String sandboxname, boolean createsandbox,
			String version,  String include, String exclude, String pattern, String replacement, String timeOut, String... filepath)

	{
		...
		addStdArguments(appname, description, createprofile, teams, criticality,  sandboxname,  createsandbox,
				 version,  include, exclude, pattern, replacement, filepath);
	}
	private void addStdArguments(String appname, String description, boolean createprofile, String teams, String criticality, String sandboxname, boolean createsandbox,
			String version, String include, String exclude, String pattern, String replacement, String... filepath) {
	 ...
	 */
	public static UploadAndScanArgs newUploadAndScanArgs(VeracodeNotifier notifier, AbstractBuild<?,?> build, hudson.EnvVars envVars, String[] filePaths, boolean bRemoteScan) {
		VeracodeDescriptor descriptor = notifier.getDescriptor();

		//credentials if(getCredentials = null) means that the credentials block is null or alternatively,
		//it means we have selected the option (use global credentials) therefore seek global credentials.
		...
		return newUploadAndScanArgs(bRemoteScan, descriptor.getAutoappname(), descriptor.getAutodescription(), descriptor.getAutoversion(), notifier.getCreatesandbox(),
				notifier.getCreateprofile(),notifier.getTeams(), descriptor.getProxy(), vId, vKey, build.getDisplayName(),
				build.getProject().getDisplayName(), notifier.getAppname(), notifier.getSandboxname(), notifier.getVersion(), notifier.getCriticality(),
				notifier.getScanincludespattern(), notifier.getScanexcludespattern(), notifier.getFilenamepattern(), notifier.getReplacementpattern(), phost,
				pport, puser, ppassword, build.getWorkspace(),  envVars,notifier.getTimeout(), filePaths);
	}
	 ...
	 * @return UploadAndScanArgs
	 */
	public static UploadAndScanArgs newUploadAndScanArgs(boolean bRemoteScan, boolean autoApplicationName, boolean autoDescription,
			boolean autoScanName, boolean createSandbox, boolean createProfile, String teams, boolean useProxy,
			String vId, String vKey, String buildNumber, String projectName, String applicationName, String sandboxName, String scanName , String criticality,
			String scanIncludesPattern, String scanExcludesPattern, String fileNamePattern, String replacementPattern,
			String pHost, String pPort, String pUser, String pCredential,
			FilePath workspace, hudson.EnvVars envVars, String timeOut, String[] filePaths) {
        ...
        //note that the API wrapper would likely not call the createbuild.do (or createapp.do) web API until a few seconds later
		...
		args.addStdArguments(
				bRemoteScan, applicationName, description, createProfile, teams, criticality, sandboxName,
				createSandbox, scanName, scanIncludesPattern, scanExcludesPattern,
				fileNamePattern, replacementPattern, timeOut, filePaths);
	...
	public static void setEnvVars(hudson.EnvVars envVars, String buildNumber, String projectName) {
        ...
        //note that the API wrapper would likely not call the createbuild.do (or createapp.do) web API until a few seconds later
        envVars.put(CUSTOM_TIMESTAMP_VAR, new java.text.SimpleDateFormat("yyyy-MM-dd-HH:mm:ss").format(new java.util.Date()));
