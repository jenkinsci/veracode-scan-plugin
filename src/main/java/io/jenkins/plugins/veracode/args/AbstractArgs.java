...
public abstract class AbstractArgs {
	...
	/**
	 * Adds the proxy configuration switches and arguments to the command line arguments
	 * list.
	 *
	 * @param phost String
	 * @param pport String
	...
	/**
	 * Adds the veracode api credentials switches and arguments to the command line arguments
	 * list.
	 *
	 * @param vid String
	 * @param vkey String
	...
	/**
	 * Returns a String that represents the plain text command line arguments
	 * added to the command line arguments list.
	 *
	 * @return String[]
	 */
	...
	/**
	 * Returns a String that represents the plain text command line arguments
	 * added to the command line arguments list, but replaces sensitive
	 * arguments with a fixed-length sequence of '*' characters.
	 * <p>
		...
		for (int x = 0; x < args.length; x++) {
			if (VKEY.equals(args[x]) || PPASSWORD.equals(args[x])) {
				if (x % 2 == 0 && x < args.length - 1 && args[x + 1] != null) {
					args[x + 1] = StringUtil.repeatChar('*', args[x + 1].length() > 0 ? MASKED_ARG_LENGTH : 0);
				}
			}
		}
		return args;
