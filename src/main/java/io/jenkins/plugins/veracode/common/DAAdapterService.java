...
public class DAAdapterService {
	...
	/**
	 * Resubmit Veracode Dynamic Analysis - A common method for both Freestyle and Pipeline
	 * @param run Run
	...
	public boolean resubmitDynamicAnalysis(Run<?, ?> run, FilePath workspace, TaskListener listener,
			final String analysisName, final int maximumDuration, final boolean failBuildAsScanFailed,
			final String apiID, final String apiKey, final boolean debugEnabled, final ProxyBlock proxyBlock) {

		try {
			log(listener, Constant.STARTING_POST_BUILD_ACTION_LOG, Constant.POST_BUILD_ACTION_DISPLAY_TEXT_RESUBMIT);
			...
			// Display user inputs
			log(listener, "Project: %s" + Constant.NEWLINE +
	                "Dynamic Analysis name: %s" + Constant.NEWLINE +
	                "Maximum duration (in hours): %s" + Constant.NEWLINE +
	                "Fail the build if the analysis fails: %s" + Constant.NEWLINE +
	                "Use proxy: %s",
	                workspace, analysisName, maximumDuration, failBuildAsScanFailed, String.valueOf(proxyBlock != null));

			// Validate user inputs
			if (!validateUserInputsForResubmit(apiID, apiKey, analysisName, maximumDuration, listener)) {
				return !failBuildAsScanFailed;
			}
			...
			DynamicAnalysisAPIService daApiService = APIServiceManager
					.createInstance(CredentialTypes.API, apiID, apiKey, proxy).getDynamicAnalysisAPIService();
			...
			if (analysisInfo == null || StringUtil.isNullOrEmpty(analysisInfo.getAnalysisId())) {
				logWithTimeStamp(listener, "Resubmit failed. Preconfigured dynamic analysis not found.");
				return !failBuildAsScanFailed;
			}
			...
			log(listener, Constant.FINISHED_POST_BUILD_ACTION_LOG, Constant.POST_BUILD_ACTION_DISPLAY_TEXT_RESUBMIT);

			return true;

        } catch (Exception e) {
            logWithTimeStamp(listener, "Resubmit failed. Unexpected error occurred: %s" + Constant.NEWLINE, e.getMessage());
            return !failBuildAsScanFailed;
        }
	}
	...
	private boolean validateUserInputsForResubmit(String apiID, String apiKey, String analysisName, int maximumDuration, TaskListener listener) {
		...
		if (StringUtil.isNullOrEmpty(apiID) || (StringUtil.isNullOrEmpty(apiKey))) {
			log(listener, "No dynamic analysis submitted. Required API ID and key credentials not provided." + Constant.NEWLINE);
			return false;
		}
		...
		if (FormValidationUtil.checkMaximumDuration(maximumDuration) != null) {
			log(listener, FormValidationUtil.checkMaximumDuration(maximumDuration) + Constant.NEWLINE);
			return false;
		}
	...
	private AnalysisInfo getAnalysisInfo(DynamicAnalysisAPIService daApiService, String analysisName, TaskListener listener) {
		try {
			...
			logWithTimeStamp(listener, "Resubmit failed. Error retrieving analysis information, server returned HTTP response code: " + e.getResponseCode());
			logErrorResponse(e.getResponseCode(), listener);
			return null;
		} catch (Exception e) {
			logWithTimeStamp(listener, "Resubmit failed. Error retrieving analysis information: %s", e.getMessage());
			return null;
		}
	}
	...
	private boolean resubmitAnalysis(DynamicAnalysisAPIService daApiService, AnalysisInfo analysisInfo, int maximumDuration, TaskListener listener) {
		try {
			...
			logWithTimeStamp(listener, "Resubmitting dynamic analysis for '%s' with duration %s hour(s).", analysisInfo.getAnalysisName(), maximumDuration);
			logWithTimeStamp(listener, "Resubmit succeeded.");
			return true;
		} catch (ApiException e) {
			logWithTimeStamp(listener, "Resubmit failed. Error submitting dynamic analysis, server returned HTTP response code: " + e.getResponseCode());
			logErrorResponse(e.getResponseCode(), listener);
			logWithTimeStamp(listener, "Analysis status for '%s' is %s", analysisInfo.getAnalysisName(), analysisInfo.getStatusInfo().getStatus() + Constant.NEWLINE);
			return false;
        } catch (Exception e) {
        	logWithTimeStamp(listener, "Resubmit failed. Unexpected error submitting dynamic analysis: %s", e.getMessage());
        	logWithTimeStamp(listener, "Analysis Status: %s", analysisInfo.getStatusInfo().getStatus() + Constant.NEWLINE);
        	return false;
        }
	}

	/**
	 * Review Veracode Dynamic Analysis Results - A common method for both Freestyle and Pipeline
	 *
	 * @param build Run
	 * @param workspace FilePath
	 * @param listener TaskListener
    ...
    public boolean reviewDynamicAnalysis(Run<?, ?> build, FilePath workspace, TaskListener listener,
			final int waitForResultsDuration, final boolean failBuildForPolicyViolation, final String apiID,
			final String apiKey, final boolean debugEnabled, final ProxyBlock proxyBlock) {

        log(listener, Constant.STARTING_POST_BUILD_ACTION_LOG, Constant.POST_BUILD_ACTION_DISPLAY_TEXT_REVIEW);
		...
		try {
            ...
            if (veracodeProps == null || veracodeProps.isEmpty()) {
            	log(listener, "Failed to retrieve dynamic analysis info from resubmit dynamic analysis step.");
            	return false;
            }
            ...
            log(listener, "Project: %s" + Constant.NEWLINE +
                    "Dynamic Analysis name: %s" + Constant.NEWLINE +
                    "Results wait time (in hours): %s" + Constant.NEWLINE +
                    "Fail the build for policy violation: %s" + Constant.NEWLINE +
                    "Use proxy: %s",
                    workspace, veracodeProps.getProperty(PARAM_DA_ANALYSIS_NAME), waitForResultsDuration, failBuildForPolicyViolation, String.valueOf(proxyBlock != null));

            // Validate user inputs
            if (!validateUserInputsForReview(apiID, apiKey, veracodeProps.getProperty(PARAM_DA_ANALYSIS_NAME), waitForResultsDuration, listener)) {
            ...
            DynamicAnalysisAPIService daApiService = APIServiceManager.createInstance(CredentialTypes.API, apiID, apiKey, proxy).getDynamicAnalysisAPIService();

            log(listener, "Requesting dynamic analysis results for '%s' with results wait time duration of %s hour(s).", veracodeProps.getProperty(PARAM_DA_ANALYSIS_NAME), waitForResultsDuration);
             ...
             *
             * Phase 3: DA is linked to an application for policy evaluation.
             * 			Get linked data including appid, appname, buildId
             * 				daApiService.getScanOccurrences(analysis occurrence id);
             *
             * The following phases use java wrapper util APIs.
             *
             * Phase 4: Wait for linking to complete which includes policy evaluation and flaw data
             * 			Get link (scan) status via getBuildInfo API and repeat until status of RESULTS_READY
             * 				WrapperUtil.getBuildInfoByAppIdBuildId(appId, buildId, true, apiID, apiKey, proxyBlock)
             *
             ...
             *
             * API exception handling: If HTTP 500 Internal Server Error or HTTP 504 Gateway Timeout response is returned from the platform,
             * then retry after poll interval for a maximum of MAX_ALLOWED_CONSECUTIVE_API_EXCEPTIONS consecutive failures.
             * If other HTTP error received, then fail the job.
             */
            ...
            try {
                ...
                String currentOccurrenceId = determineAnalysisOccurrenceId(daApiService, veracodeProps.getProperty(PARAM_DA_ANALYSIS_NAME), veracodeProps.getProperty(PARAM_DA_PREVIOUS_OCCURRENCE_ID), expirationResultsWaitTime, listener);
                ...
                isAnalysisFinished = waitForAnalysisToComplete(daApiService, currentOccurrenceId, expirationResultsWaitTime, listener);
                if (!isAnalysisFinished) {
                    logWithTimeStamp(listener, "Timeout waiting for dynamic analysis to complete and publish results.");
                    build.addAction(new DynamicAnalysisResultsAction());
                    return false;
                }

                // Phase 3:  Wait for linked results of app id, app name, build id to be available
                // If wait for results duration expired, then fail job.
                scanOccurrenceInfo = getLinkedAnalysisResults(daApiService, currentOccurrenceId, expirationResultsWaitTime, listener);
                if (null == scanOccurrenceInfo) {
                    logWithTimeStamp(listener, "Timeout waiting for dynamic analysis link results.");
                    build.addAction(new DynamicAnalysisResultsAction());
                    return false;
                }
            } catch (InterruptedException e) {
                logWithTimeStamp(listener, "Interrupted exception handling dynamic analysis action.");
                build.addAction(new DynamicAnalysisResultsAction());
                e.printStackTrace();
                return false;
            } catch (RuntimeException e) {
                logWithTimeStamp(listener, "Runtime exception error handling dynamic analyis action: %s", e.getMessage());
                e.printStackTrace();
                build.addAction(new DynamicAnalysisResultsAction());
                return false;
            } catch (Exception e) {
                logWithTimeStamp(listener, "Exception handling dynamic analysis action: %s", e.getMessage());
                e.printStackTrace();
                build.addAction(new DynamicAnalysisResultsAction());
                return false;
            }
            ...
            try {
				...
				// Phase 4:  Wait for application build to complete. Analysis is linked to Veracode application for policy evaluation.
				boolean isBuildReady = waitForBuildReady(scanOccurrenceInfo, apiID, apiKey, proxyBlock, expirationResultsWaitTime, listener);
				if (!isBuildReady) {
				...
				// Phase 5:  Get the detailed report. Need flaw counts by severity, score, policy status, mitigated findings.
				...
				log(listener, Constant.NEWLINE + "The Dynamic Analysis finished with policy rule status: %s", policyRulesStatus + Constant.NEWLINE);

				// Create action for the results graph
				build.addAction(new DynamicAnalysisResultsAction(daScanHistory));

            } catch (ApiException e) {
                logWithTimeStamp(listener, "API exception while waiting for Dynamic Analysis linking results status.");
                build.addAction(new DynamicAnalysisResultsAction());
                return false;
            } catch (InterruptedException e) {
                logWithTimeStamp(listener, "Interrupted exception handling dynamic analysis linking results action.");
                build.addAction(new DynamicAnalysisResultsAction());
                e.printStackTrace();
                return false;
            } catch (RuntimeException e) {
                logWithTimeStamp(listener, "Runtime exception error handling dynamic analyis linking results action: %s", e.getMessage());
                e.printStackTrace();
                build.addAction(new DynamicAnalysisResultsAction());
                return false;
            }

            log(listener, Constant.FINISHED_POST_BUILD_ACTION_LOG, Constant.POST_BUILD_ACTION_DISPLAY_TEXT_REVIEW);

            return policyRulesStatus.equalsIgnoreCase(Constant.PASSED) ? true : !failBuildForPolicyViolation;
    ...
    private boolean validateUserInputsForReview(String apiID, String apiKey, String analysisName, int waitForResultsDuration, TaskListener listener) {
    	// Validate API credentials
        if (StringUtil.isNullOrEmpty(apiID) || (StringUtil.isNullOrEmpty(apiKey))) {
            log(listener, "Error requesting Dynamic Analysis results - required API ID and key credentials not provided");
            return false;
        }
        ...
        if (FormValidationUtil.checkWaitForResultsDuration(waitForResultsDuration) != null) {
            ...
            log(listener, FormValidationUtil.checkWaitForResultsDuration(waitForResultsDuration) + Constant.NEWLINE);
            return false;
        }

        // Validate Dynamic Analysis Name
        if (StringUtil.isNullOrEmpty(analysisName)) {
            log(listener, "Error requesting Dynamic Analysis results - Dynamic Analysis scan name is unknown.");
            log(listener, "Verify Resubmit Veracode Dynamic Analysis post build action ran successfully in this Jenkins build prior to requesting results." + Constant.NEWLINE);
            return false;
        }
    ...
    private String determineAnalysisOccurrenceId(DynamicAnalysisAPIService daApiService, String analysisName, String previousOccurrenceId, long expirationResultsWaitTime, TaskListener listener) throws Exception {
       ...
       while (!isAnalysisCreated && !isTimeToQuit) {
          try {
              ...
              currentOccurrenceId = analysisInfo != null ? analysisInfo.getAnalysisOccurrenceId() : null;
              ...
              if (StringUtil.isNullOrEmpty(currentOccurrenceId) || currentOccurrenceId.equals(previousOccurrenceId)) {
                  logWithTimeStamp(listener, "Dynamic analysis not yet initiated. Check in %s minutes.", GET_DA_SLEEP_TIME_MINUTES);
              ...
              log(listener, "API exception error waiting for dynamic analysis to initiate. Server returned HTTP response code: " + e.getResponseCode());
              logErrorResponse(e.getResponseCode(), listener);
              // Retry if internal server or gateway timeout error
              if ((500 == e.getResponseCode()) || (504 == e.getResponseCode())) {
                  exceptionCount++;
                  if ((exceptionCount == MAX_ALLOWED_CONSECUTIVE_API_EXCEPTIONS) || isWaitTimeDurationExpired(expirationResultsWaitTime)) {
                      isTimeToQuit = true;
                  } else {
                      logWithTimeStamp(listener, "Retry checking if dynamic analysis initiated in %s minutes.", GET_DA_SLEEP_TIME_MINUTES);
                      Thread.sleep(TimeUnit.MINUTES.toMillis(GET_DA_SLEEP_TIME_MINUTES));
                      isTimeToQuit = isWaitTimeDurationExpired(expirationResultsWaitTime);
                  }
              } else {
       ...
       while (!isAnalysisFinished && !isTimeToQuit) {
          ...
          try {
              ...
              if (analysisOccurrenceInfo == null) {
                  logWithTimeStamp(listener, "Dynamic analysis occurrence not found. Check in %s minutes.", GET_DA_SLEEP_TIME_MINUTES);
                  Thread.sleep(TimeUnit.MINUTES.toMillis(GET_DA_SLEEP_TIME_MINUTES));
                  isTimeToQuit = isWaitTimeDurationExpired(expirationResultsWaitTime);
                  continue;
              }

              StatusTypeEnum analysisStatus = analysisOccurrenceInfo.getAnalysisStatus().getStatus();

              // Dynamic analysis complete and results published. Next step is linking the analysis results to an application.
              if (analysisStatus != null && analysisStatus.equals(StatusTypeEnum.FINISHED_RESULTS_AVAILABLE)) {
                  isAnalysisFinished = true;
                  logWithTimeStamp(listener, "The status of the dynamic analysis is: %s", analysisStatus);
                  logWithTimeStamp(listener, "The dynamic analysis finished with occurrence id: %s", currentOccurrenceId);
                  logWithTimeStamp(listener, "The next step is linking the analysis to the application for policy evaluation.");
              } else if (analysisStatus != null && (analysisStatus.equals(StatusTypeEnum.VERIFICATION_FAILED)
            	  ...
            	  logWithTimeStamp(listener, "The dynamic analysis failed to complete with status: %s", analysisStatus);
            	  isTimeToQuit = true;
              } else {
                  logWithTimeStamp(listener, "The status of the dynamic analysis is: %s"
                     + Constant.NEWLINE + timestamp() + "Requesting status in %s minutes", analysisStatus, GET_DA_SLEEP_TIME_MINUTES);
                  Thread.sleep(TimeUnit.MINUTES.toMillis(GET_DA_SLEEP_TIME_MINUTES));
                  isTimeToQuit = isWaitTimeDurationExpired(expirationResultsWaitTime);
              }

          } catch (ApiException e) {
              log(listener, "API exception error retrieving scan analysis status. Server returned HTTP response code: " + e.getResponseCode());
              logErrorResponse(e.getResponseCode(), listener);
              ...
              if ((500 == e.getResponseCode()) || (504 == e.getResponseCode())) {
                  ...
                  if ((exceptionCount == MAX_ALLOWED_CONSECUTIVE_API_EXCEPTIONS) || isWaitTimeDurationExpired(expirationResultsWaitTime)) {
                      ...
                      logWithTimeStamp(listener, "Retry requesting scan occurrence info in %s minutes.", GET_DA_SLEEP_TIME_MINUTES);
                      Thread.sleep(TimeUnit.MINUTES.toMillis(GET_DA_SLEEP_TIME_MINUTES));
                      isTimeToQuit = isWaitTimeDurationExpired(expirationResultsWaitTime);
                  }
              } else {
	...
	private ScanOccurrenceInfo getLinkedAnalysisResults(DynamicAnalysisAPIService daApiService, String occurrenceId, long expirationResultsWaitTime, TaskListener listener) throws Exception {
        ...
        while (!isLinkedAppInfoReady && !isTimeToQuit) {
            ...
            try {
                Set<ScanOccurrenceInfo> scanOccurrenceInfoSet = daApiService.getScanOccurrences(occurrenceId);
                ...
                if (scanOccurrenceResultsInfo != null) {
                    ...
                    if (StringUtil.isNullOrEmpty(scanOccurrenceResultsInfo.getLinkedPlatformAppId())) {
                        logWithTimeStamp(listener, "Review results failed. Linked application is unknown.");
                        logWithTimeStamp(listener, "Verify dynamic analysis is linked to an application.");
                        return null;
                    }
                    // the linking phase may be delayed depending on platform load, so need to wait for
                    // build id to be populated when linking initiated
                    if ((null != scanOccurrenceResultsInfo.getLinkedAppData()) &&
                        (!StringUtil.isNullOrEmpty(scanOccurrenceResultsInfo.getLinkedAppData().getBuildId()))) {
                        isLinkedAppInfoReady = true;
                        logWithTimeStamp(listener, "The linked application is: %s (appid=%s)", scanOccurrenceResultsInfo.getLinkedPlatformAppName(), scanOccurrenceResultsInfo.getLinkedPlatformAppId());
                        logWithTimeStamp(listener, "The linked application build ID is: %s", scanOccurrenceResultsInfo.getLinkedAppData().getBuildId());
                    } else {
                        logWithTimeStamp(listener, "Build id is not available." + Constant.NEWLINE + timestamp() + "Requesting build id again in %s minute(s).", GET_DA_SLEEP_TIME_MINUTES);
                        Thread.sleep(TimeUnit.MINUTES.toMillis(GET_DA_SLEEP_TIME_MINUTES));
                    }
                ...
                log(listener, "API exception error retrieving scan occurrence info. Server returned HTTP response code: " + e.getResponseCode());
                logErrorResponse(e.getResponseCode(), listener);
    ...
    private boolean waitForBuildReady(ScanOccurrenceInfo scanOccurrenceInfo, String apiID, String apiKey, ProxyBlock proxyBlock, long expirationResultsWaitTime, TaskListener listener) throws Exception {
        ...
        while (!isBuildReady && !isTimeToQuit) {
           try {
                String buildInfo = WrapperUtil.getBuildInfoByAppIdBuildId(scanOccurrenceInfo.getLinkedPlatformAppId(),
                   scanOccurrenceInfo.getLinkedAppData().getBuildId(), apiID, apiKey, proxyBlock);

                if (!StringUtil.isNullOrEmpty(buildInfo)) {

                   Node nodeAnalysisUnit = XmlUtils.getXmlNode(buildInfo, "/*/*/*[local-name()='analysis_unit']");
		           String buildStatus = nodeAnalysisUnit.getAttributes().getNamedItem("status").getNodeValue();

                   if (buildStatus != null && buildStatus.equalsIgnoreCase(Constant.RESULTS_READY)) {
                       logWithTimeStamp(listener, "Dynamic analysis linking is complete with status: %s", buildStatus);
                       isBuildReady = true;
                   // linked results are not yet ready so retry
                   } else {
                       logWithTimeStamp(listener, "The linking status of the dynamic analysis is: %s"
                         + Constant.NEWLINE + timestamp() + "Requesting linking status in %s minutes", buildStatus, GET_DA_SLEEP_TIME_MINUTES);
                       Thread.sleep(TimeUnit.MINUTES.toMillis(GET_DA_SLEEP_TIME_MINUTES));
                       isTimeToQuit = isWaitTimeDurationExpired(expirationResultsWaitTime);
                   }
               } else {
	           ...
	           log(listener, "API exception error retrieving linked results status. Server returned HTTP response code: " + e.getResponseCode());
               logErrorResponse(e.getResponseCode(), listener);
               ...
               if ((500 == e.getResponseCode()) || (504 == e.getResponseCode())) {
                   ...
                   if ((exceptionCount == MAX_ALLOWED_CONSECUTIVE_API_EXCEPTIONS) || isWaitTimeDurationExpired(expirationResultsWaitTime)) {
                       ...
                       logWithTimeStamp(listener, "Retry requesting linked status in %s minutes.", GET_DA_SLEEP_TIME_MINUTES);
                       Thread.sleep(TimeUnit.MINUTES.toMillis(GET_DA_SLEEP_TIME_MINUTES));
                       isTimeToQuit = isWaitTimeDurationExpired(expirationResultsWaitTime);
                   }
               } else {
